---
name: claude-code
description: Delegate coding work to the Claude Code CLI from any agent or harness — one-shot print-mode runs, multi-turn tmux sessions, PR reviews, parallel workers. Use when asked to hand a feature, refactor, bug fix, or code review to Claude Code, or when you need an autonomous coding subprocess that can read, edit, run commands, and manage git.
---
...
This guide is harness-agnostic: everything is expressed as shell commands you run with whatever command-execution tool you have.

## Harness mapping

You need three capabilities. Map them to whatever your harness provides:

| Need | Generic form | Typical mappings |
|------|--------------|------------------|
| Run a command in a directory | `cd <dir> && <cmd>` | a shell/exec tool with a `cwd`/`workdir` argument |
| Run something long in the background | background flag, or `nohup <cmd> > /tmp/run.log 2>&1 &`, or tmux | `background=true`, `run_in_background`, `&` + log file |
| Drive an interactive TUI | tmux `send-keys` / `capture-pane` | a PTY-capable exec tool, or tmux (works everywhere) |

**tmux is the portable fallback.** If your harness has no PTY or process-control tool, every interactive pattern below still works through tmux alone. If you have a dedicated process tool (poll/log/submit/kill), use it instead of `capture-pane`/`send-keys`.
...
OAuth credentials live in the invoking user's home directory. If your harness runs as a different user or with a rewritten `HOME`, auth will appear missing — verify with `claude auth status` before blaming the task.
...
```bash
cd /path/to/project && claude -p 'Add error handling to all API calls in src/' \
...
Interactive mode gives you a full conversational REPL where you can send follow-up prompts, use slash commands, and watch Claude work in real time. **Requires a PTY; tmux is the reliable way to get one.**
...
Claude Code presents up to two confirmation dialogs on first launch. You MUST handle these.
...
❯ 1. Yes, I trust this folder    ← DEFAULT (just press Enter)
  2. No, exit
```
**Handling:** `tmux send-keys -t <session> Enter` — the default selection is correct.
...
❯ 1. No, exit                    ← DEFAULT (WRONG choice!)
  2. Yes, I accept
```
**Handling:** navigate DOWN first, then Enter:
```bash
...
# Trust dialog (Enter for default "Yes")
sleep 4 && tmux send-keys -t claude-work Enter

# Permissions dialog (Down then Enter for "Yes, I accept")
...
```bash
# Start a task
cd /project && claude -p 'Start refactoring the database layer' \
  --output-format json --max-turns 10 > /tmp/session.json

# Resume with session ID
cd /project && claude -p 'Continue and add connection pooling' \
  --resume "$(jq -r .session_id /tmp/session.json)" --max-turns 5
...
```bash
cd /project && claude --bare -p 'Run all tests and report failures' \
...
| Flag | Effect |
|------|--------|
| `--model <alias>` | Model selection: `sonnet`, `opus`, `haiku`, or a full model name |
...
| `--system-prompt <text>` | **Replace** the entire system prompt (usually prefer `--append`) |
...
| Flag | Effect |
|------|--------|
| `-d, --debug [filter]` | Debug logging with optional category filter (e.g., `"api,hooks"`, `"!1p,!file"`) |
...
| `/voice` | Push-to-talk voice mode (hold Space to record, release to send) |
...
Unlike slash commands (manually invoked), skills in `.claude/skills/` are markdown guides Claude invokes automatically via natural language when the task matches:
...
| Prefix | Action |
|--------|--------|
| `!` | Execute bash directly, bypassing the model (e.g., `!npm test`); `!` alone toggles shell mode |
| `@` | Reference files/directories with autocomplete (e.g., `@./src/api/`) |
| `#` | Quick add to CLAUDE.md memory (e.g., `# Use 2-space indentation`) |
| `/` | Slash commands |

**Pro tip:** the keyword "ultrathink" in a prompt triggers maximum reasoning effort for that turn, regardless of the current `/effort` setting.
...
cd /path/to/repo && git diff main...feature-branch | \
  claude -p 'Review this diff for bugs, security issues, and style problems. Be thorough.' --max-turns 1
```
...
Run multiple independent Claude tasks simultaneously — one tmux session (or one background process) per task:

```bash
tmux new-session -d -s task1 -x 140 -y 40 && tmux send-keys -t task1 \
  'cd ~/project && claude -p "Fix the auth bug in src/auth.py" --allowedTools "Read,Edit" --max-turns 10' Enter

tmux new-session -d -s task2 -x 140 -y 40 && tmux send-keys -t task2 \
  'cd ~/project && claude -p "Write integration tests for the API endpoints" --allowedTools "Read,Write,Bash" --max-turns 15' Enter

tmux new-session -d -s task3 -x 140 -y 40 && tmux send-keys -t task3 \
  'cd ~/project && claude -p "Update README.md with the new API endpoints" --allowedTools "Read,Edit" --max-turns 5' Enter

# Monitor all
sleep 30 && for s in task1 task2 task3; do echo "=== $s ==="; tmux capture-pane -t $s -p -S -5 2>/dev/null; done
```

If your harness has native background execution, use it instead — same commands, redirect each to its own log file, and poll the logs.

Give parallel workers **separate working directories** (git worktrees) when they write to the same repo, or they will fight over the index.
...
- Separate from CLAUDE.md — these are Claude's own notes, accumulated across sessions

## Custom Subagents

Define specialized agents in `.claude/agents/` (project), `~/.claude/agents/` (personal), or via `--agents` (session).
...
- **Result size:** Default capped; use the `maxResultSizeChars` annotation to allow up to **500K** characters
- **Output tokens:** `export MAX_MCP_OUTPUT_TOKENS=50000` — cap MCP output to prevent context flooding
...
- `❯` at bottom = waiting for input (Claude is done or asking a question)
- `●` lines = Claude is actively using tools
- `⏵⏵ bypass permissions on` = permissions mode in the status bar
...
| `MAX_THINKING_TOKENS` | Cap thinking tokens (`0` disables thinking entirely) |
| `MAX_MCP_OUTPUT_TOKENS` | Cap output from MCP servers (e.g., `50000`) |
| `CLAUDE_CODE_NO_FLICKER=1` | Alt-screen rendering to eliminate terminal flicker |
...
2. **Use `--max-budget-usd`** for cost caps. Minimum is ~$0.05 (system prompt cache creation).
...
8. **Use `--model haiku`** for simple tasks and `--model opus` for complex multi-step work.
9. **Use `--fallback-model haiku`** in print mode to handle model overload gracefully.
10. **Start new sessions for distinct tasks** — fresh context is more efficient.
11. **Use `--no-session-persistence`** in CI to avoid accumulating saved sessions on disk.

## Pitfalls & Gotchas

1. **Interactive mode needs a real PTY** — Claude Code is a full TUI app. tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is what makes orchestration reliable.
2. **`--dangerously-skip-permissions` dialog defaults to "No, exit"** — send Down then Enter to accept. Print mode (`-p`) skips it entirely.
3. **`--max-budget-usd` minimum is ~$0.05** — system prompt cache creation alone costs that much; lower values error immediately.
4. **`--max-turns` is print-mode only** — ignored in interactive sessions.
5. **Claude may use `python` instead of `python3`** — on systems without a `python` symlink the first bash attempt fails, then it self-corrects.
6. **Session resumption is directory-scoped** — `--continue` finds the most recent session for the current working directory.
7. **`--json-schema` needs enough `--max-turns`** — Claude must read files before producing structured output.
8. **Trust dialog appears once per directory** — first time only, then cached.
9. **Background tmux sessions persist** — always clean up with `tmux kill-session -t <name>`.
10. **Slash commands only work in interactive mode** — in `-p` mode, describe the task in natural language.
11. **`--bare` skips OAuth** — requires `ANTHROPIC_API_KEY` or an `apiKeyHelper` in settings.
12. **Context degradation is real** — output quality measurably degrades above 70% context usage. Monitor and `/compact` proactively.

## Rules for Orchestrating Agents

1. **Prefer print mode (`-p`) for single tasks** — cleaner, no dialog handling, structured output.
2. **Use tmux (or your PTY tool) for multi-turn work** — the only reliable way to drive the TUI.
3. **Always set the working directory** — keep Claude focused on the right project.
4. **Set `--max-turns` in print mode** — prevents infinite loops and runaway costs.
5. **Monitor long runs** — `tmux capture-pane -t <session> -p -S -50`, or tail the background log.
6. **Look for the `❯` prompt** — Claude is waiting for input (done, or asking a question).
7. **Clean up sessions** — kill tmux sessions and background processes when done.
8. **Report results to the user** — summarize what Claude did and what changed.
9. **Don't kill slow sessions** — multi-step work takes time; check progress first.
