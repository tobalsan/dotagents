...
Routing does not complete execution: after routing, load the selected autonomous-agent skill and obey its process, timeout, and cleanup guidance. Launch from the smallest common ancestor containing approved inputs and the campaign-local output directory. Do not configure a harness single-output override; stdout and stderr are diagnostic streams, while node-owned artifacts remain authoritative. The orchestrator enforces `timeout_seconds`, `max_concurrency`, and budget limits. On cancellation or timeout it requests termination, waits a bounded grace period, then terminates the full owned process tree and reaps it. It must not kill unrelated processes. Partial artifacts may remain for diagnosis but are not successful outputs.
