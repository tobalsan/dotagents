...
The orchestrator supplies only dependencies whose manifest state permits this node to run. Together, dependency result paths and campaign-state paths are the attempt's exact readable-path allowlist; workspace reconnaissance and undeclared reads are forbidden. Prior-attempt paths are denied by default. The sole exception is a `repair.mode: contract_only` envelope prepared after the orchestrator has proven a format-only defect and recorded `invalid_node_result_format`: it has empty dependencies, campaign state, and retrieval skills, and its `repair.readable_paths` names only files inside the immediately preceding attempt. Input paths are artifact-root-relative, must not escape that root after symlink resolution, and must not overlap `output_dir`. Reusing a directory across nodes or attempts is forbidden.
...
Routing does not complete execution: after routing, load the selected autonomous-agent skill and obey its process, timeout, and cleanup guidance. Before every launch, run `scripts/prepare_node_contract.py` for the exact `node-input.json`. It places the exact schema, identity-filled result template, validator, `worker-contract.json`, and executable `self-validate.sh` in `output_dir`. The worker prompt must explicitly require reading `worker-contract.json`, the schema, and the template before substantive work, and running `./self-validate.sh` before exit; the worker may not report completion unless it prints `"valid":true`. Missing contract packet or missing successful self-validation blocks launch or acceptance. Launch from the smallest common ancestor containing approved inputs and the campaign-local output directory. Do not configure a harness single-output override; stdout and stderr are diagnostic streams, while node-owned artifacts remain authoritative. The orchestrator enforces `timeout_seconds`, `max_concurrency`, and budget limits.
...
Do not rerun expensive work for a format-only rejection. When independent validation proves substantive evidence and files complete and fails solely on field names, types, required/extra fields, or envelope shape, record the distinct orchestration error code `invalid_node_result_format`, append failed-with-retry, then run `prepare_retry.py --contract-repair` before appending `retrying`. Generic `invalid_node_result` is not repairable. Supply `--repair-read` only for the prior `node-result.json` and declared prior artifact files needed to normalize it. The generated attempt has no dependencies, campaign state, or retrieval skills. Route it to a cheap model and enforce a harness tool allowlist permitting only declared-file reads and fresh-output writes. Prohibit retrieval, network, workspace discovery, and subagents at both prompt and tool-policy layers; block repair if the selected harness cannot enforce that boundary. Permit only schema normalization and copying declared evidence into its fresh output directory. Identity mismatch, missing evidence, path escape, citation/source disagreement, nonzero completion, timeout, or substantive failure is not repairable; use an ordinary isolated retry or fail.
