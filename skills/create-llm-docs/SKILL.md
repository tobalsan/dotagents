---
name: create-llm-docs
description: Generate a single token-efficient `docs/llms.md` onboarding brief for a codebase, optimized for LLM/coding-agent consumption. Use when the user asks to create LLM docs, agent onboarding docs, an llms.md, a coding-agent brief, or wants to document a repo so another AI agent can quickly orient, reason about architecture, and make safe changes.
---

# Create LLM Docs

Generate a single high-signal `docs/llms.md` Markdown file that serves as a compact but comprehensive operating brief for an LLM/coding agent that WILL have direct access to the repository.

## Primary Goal

Create an onboarding brief so a new coding agent can quickly:
- Reason about the project
- Make safe changes
- Avoid breaking invariants

Prioritize problem-solving rationale, architecture, design decisions, and practical change workflows over exhaustive description.
...
- Write the file to `docs/llms.md` at the repo root being documented. Create the `docs/` directory if it does not exist.
- The file content MUST be Markdown only — no surrounding backticks, no extra commentary.
- Optimize for token efficiency:
  - Dense headings + bullets, not long paragraphs.
  - No repeated facts across sections.
  - Prefer file paths and symbol names over prose.
  - Include only details that change implementation decisions.
- Include concrete repo-relative file references.
- Do NOT include secrets, API keys, passwords, private URLs, or copied env contents.

## Required Structure

The generated `docs/llms.md` MUST follow this exact section structure and ordering.
...
1. Inspect the repository directly (top-level config, entrypoints, package manifests, key modules, tests, CI configs, migrations, infra).
2. Extract architecture, rationale, and operating constraints from code evidence — not assumptions.
3. Draft the `docs/llms.md` content following the required structure above.
4. Verify no secrets or sensitive values are included.
5. Write the file to `docs/llms.md` at the repo root. Create `docs/` if missing.
6. Report the path to the user.
